...
class MLP:
    ...
    def __init__(self, hidden_activation_function_list,M = 64,  parameter_init_type = "RANDOM", num_hidden_layers=1,  output_activation_function = af.logistic):
        ...
        #instead just represent  ALL layers as a list or array of Layer objects, that last will be out output
        self.layers = [num_hidden_layers]
         
        self.hidden_layers= self.hidden_layers(num_hidden_layers, hidden_activation_function_list) #LIST of hidden activation functions: change later to incor mult inner activations, so we can adjust depth
